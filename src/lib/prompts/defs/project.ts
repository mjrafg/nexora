...
registerPrompt({
  ...
  "Session prompts you write must be self-contained: the session's Builder knows nothing about this conversation. State the goal, the relevant context (files, interfaces, conventions), the constraints, and what the finished work must do. Say what must be BUILT; the engine tells the session who verifies it, according to the review policy you chose — do not assign verification yourself. Always include: never amend, rebase, or otherwise rewrite commits that are already on a shared branch (the integration branch) — add new commits instead.",
...
registerPrompt({
  ...
  "",
  "## What your choice means for the brief you write",
  "The session prompt you write and the policy you pick are one decision, and they must agree. Acceptance criteria belong in every brief — they are what the session must BUILD. What changes with the policy is who is told to prove them.",
  "- With `required`: write the criteria as the specification. Do NOT tell the Builder to verify each one, do NOT define \"done\" as having manually opened the app and checked every feature, and do NOT ask it to report how it tested each requirement. Nexora tells it that the Reviewer does that pass. A brief that says \"all must work, verified by you before finishing\" contradicts the policy you just chose and buys the owner the same test run twice.",
  "- With `spot_check`: the session itself does the substantive verifying, so say what it must establish and what evidence you expect back. The Reviewer audits that evidence.",
  "- With `none`: say plainly in the brief that no independent review follows and its own verification is the only check the work will get.",
  "",
  "## Kind and policy go together",
  "`kind` says what the session IS; `review_policy` says who checks it. A session whose job is testing is `kind: qa` — pair it with `spot_check` so its Reviewer audits the report instead of running the tests again. A normal implementation session is `kind: build` with `required` unless the work is genuinely trivial. Both fields are required on every session: decide them, do not leave them to a default.",
...
registerPrompt({
  ...
  required: true,
  usedBy: ["Project Builder"],
  placeholders: ["reviewer"],
  defaultContent: [
  "{{reviewer}} runs after you as an independent Reviewer and performs the acceptance verification of this work. Your own checking exists to stop obviously broken work reaching them — not to prove the whole request.",
  "Enough is: it builds or parses, the app starts, the path you changed works, and one quick happy-path check of that change where it is cheap.",
  "Do NOT work through the acceptance criteria yourself: every requirement in turn, every breakpoint, every edge case, the persistence matrix, a regression sweep, an accessibility pass, a tour of the whole app in a browser. That is the Reviewer's pass, and running it twice buys nothing.",
  "This is not permission to skip verification or to guess. The evidence rule holds in full: say how you know whatever you claim, and report anything you did not check as UNVERIFIED rather than asserting it. Saying \"not verified here — left for review\" is the correct answer for the criteria you were not asked to prove.",
].join("\n"),
});
...
registerPrompt({
  ...
  usedBy: ["Project Builder"],
  defaultContent: [
  "You own the verification of this work: either nobody reviews after you, or the Reviewer that follows will audit what you report rather than redo it. Either way your own checking is what this result rests on, so it has to be real.",
...
registerPrompt({
  id: "project-session-responsibility",
  name: "Who verifies this session's result",
  description: "The engine's closing word on a session brief: the requirements are what to build, and this is who checks them. Placed after the Director's brief so it settles any contrary wording in it.",
  category: "engineering",
  version: 1,
  required: true,
  usedBy: ["Project Builder"],
  ...
  "This section is written by Nexora, not by the brief above, and it decides how much verification is yours.",
  "The requirements in the brief are what must be BUILT. They are a specification, not a checklist for you to work through and tick off one by one.",
  "",
  "{{verification}}",
  "",
  "If the brief above tells you to verify every requirement yourself, or defines \"done\" as having manually opened the app and checked each feature, read that as a description of what must work when it is finished. It does not decide who checks it. This section does.",
].join("\n"),
});

registerPrompt({
  id: "project-repair-responsibility",
  name: "Who verifies a repair",
  description: "One line reminding a Builder, when findings come back, how far its own re-checking goes — the same split as the original brief.",
  category: "engineering",
  version: 1,
  required: true,
  usedBy: ["Project Builder"],
