/* ------------------------------------------------------------------
   Reading the Reviewer's reply.

   Import-free on purpose: this is the piece that decides whether work
   gets sent back for repair, and it once decided that a Reviewer saying
   "I've requested command-execution access and will resume" meant the
   code had a major defect. It is worth being able to check on its own.
   ------------------------------------------------------------------ */

import type { Finding, ReviewVerdict } from "./types";

/**
 * The synthetic finding the parser produces when a reply answers no contract.
 *
 * Named so the engine can recognise its own guess and decline to act on it —
 * a reply that is merely unparseable is not evidence that the code is wrong.
 */
export const UNSTRUCTURED_TITLE = "Reviewer reported issues (unstructured output)";

/** Did the parser understand this reply, or only fail to? */
export function isUnstructuredGuess(items: Finding[]): boolean {
  return items.length === 1 && items[0].title === UNSTRUCTURED_TITLE;
}
...
export function parseVerdict(text: string): { verdict: ReviewVerdict; items: Finding[] } {
  ...
  if (items.length === 0) {
    ...
    items.push({ severity: "major", title: UNSTRUCTURED_TITLE, detail: text.trim().slice(0, 4_000) });
  }
