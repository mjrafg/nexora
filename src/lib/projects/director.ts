...
const DIRECTOR_TOOLS: { name: string; op: string; description: string; inputSchema: Record<string, unknown> }[] = [
  ...
  { name: "plan_milestone_sessions", op: "plan_sessions", description: "Decompose one milestone into self-contained sessions.", inputSchema: { type: "object", properties: { milestone: { type: "string" }, reasoning: { type: "string" }, sessions: { type: "array", items: { type: "object", properties: { key: { type: "string" }, name: { type: "string" }, purpose: { type: "string" }, prompt: { type: "string" }, depends_on: { type: "array", items: { type: "string" } }, isolated: { type: "boolean" }, agent_id: { type: "string" }, skills: { type: "array", items: { type: "string" }, description: "skill_ids from list_skills that this session's Builder should be given — choose from the work, usually none, one or two" }, reviewer_skills: { type: "array", items: { type: "string" }, description: "skill_ids for the independent Reviewer of this session; usually different from the Builder's" }, kind: { type: "string", enum: ["build", "qa", "cleanup"], description: "what this session IS. build = it implements something. qa = its job is testing/verifying work that already exists, however it is named — a QA pass, a test sweep, a verification session, 'test and fix' — choose qa whenever the session's purpose is to check rather than to create. cleanup = a small docs or tidy-up change. Required: decide it, do not leave it out." }, review_policy: { type: "string", enum: ["required", "spot_check", "none"], description: "who checks this session's result. required = an independent Reviewer performs the acceptance pass (so your prompt must NOT tell the Builder to verify every requirement itself); spot_check = the session verifies its own work and the Reviewer audits that evidence — the right pairing for kind: qa; none = no Reviewer at all. Required: decide it, do not leave it out." }, review_why: { type: "string", description: "one line: why that review policy is right for this session. Recorded in the project audit trail." }, grant_tools: { type: "array", items: { type: "string" }, description: "extra permissions this session's Builder needs, beyond what its agent already has: read_files, write_files, run_commands, web_search, web_fetch, browser, company_profile. Scoped to this session only. Anything else (payments, credentials, writing company data) is refused — those go through the Capability Manager, which asks the owner." }, grant_servers: { type: "array", items: { type: "string" }, description: "tool servers this session may use, by name. Only servers someone on this project has already been given." } }, required: ["key", "name", "purpose", "prompt", "kind", "review_policy"] } } }, required: ["milestone", "sessions", "reasoning"] } },
  ...
  try {
    switch (op) {
      ...
      case "plan_sessions": {
        ...
        /*
         * A schema's `required` is advice to a model; this is the gate.
         *
         * Both fields had defaults, and a QA session — "Thoroughly test the
         * running app against every product requirement… produce a QA report"
         * — was filed as kind `build` simply because the field was omitted.
         * Its Reviewer then had no way to know the testing had already been
         * done. The default was doing the deciding, so there is no default.
         */
        const undecided = raw.filter((s) => !KINDS.has(String(s.kind)) || !POLICIES.has(String(s.review_policy)));
        if (undecided.length) {
          return {
            ok: false,
            error: [
              `These sessions are missing a deliberate choice: ${undecided.map((s) => String(s.key)).join(", ")}.`,
              "Every session needs both:",
              "- kind: build (it implements something) · qa (its job is testing work that already exists) · cleanup (small docs or tidy-up)",
              "- review_policy: required (an independent Reviewer does the acceptance pass) · spot_check (the session verifies its own work, the Reviewer audits it — pair this with kind qa) · none (no Reviewer)",
              "Reissue plan_milestone_sessions with both set on every session. They decide who verifies the work, so the engine will not pick for you.",
            ].join("\n"),
          };
        }
      ...
      case "deliver": {
        ...
        const coverage = reviewCoverageNote(readDb().projectSessions.filter((x) => x.projectId === projectId));
